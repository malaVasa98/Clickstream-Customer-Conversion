...
if uploaded_file is not None:
    ...
    if st.button("Run app"):
        ...
        # Load the model
        loaded_xgb = joblib.load("XGBoost_regress.pkl")

        # Load the Scaler
        loaded_reg_sc = joblib.load("Regress_scale.pkl")
        
        test_click_regress = test_click.drop(['year','order','session_id','page2_clothing_model'],axis=1)
        X_unk = test_click_regress.copy()
        X_sc_unk = loaded_reg_sc.transform(X_unk.values)
        Pred_price = loaded_xgb.predict(X_sc_unk)
        ...
        # Load the model
        loaded_log_reg = joblib.load("Classify_logistic.pkl")

        # Load the scaler
        loaded_class_sc = joblib.load("Classify_scaler.pkl")
        X_cl_unk = test_click_class.copy()
        X_cl_sc = loaded_class_sc.transform(X_cl_unk.values)
        Pred_price_2 = loaded_log_reg.predict(X_cl_sc)
        ...
        # Load the model
        loaded_km = joblib.load("Cluster_best.pkl")

        # Load the dim reducer
        loaded_umap = joblib.load("Umap_reducer.pkl")

        # Load the scaler
        loaded_clust_sc = joblib.load("Scale_cluster.pkl")
        
        # Cluster on unknown data 
        cluster_scaled_unk = pd.DataFrame(loaded_clust_sc.transform(cluster_click_unk),
                                          columns=cluster_click_unk.columns,
                                          index=cluster_click_unk.index)
        
        cluster_umap_unk = loaded_umap.transform(cluster_scaled_unk)
        labels_unk = loaded_km.predict(cluster_umap_unk)
